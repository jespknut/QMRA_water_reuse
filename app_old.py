        ...
        **Median total annual risk of infection:** {p50_risk:.2e}
        ...
        if 'summary_text' in locals() and 'ai_summary' in locals():
            docx_file = create_docx_report(
                ...
                fig,
                fig2,
                fig_cases=fig_cases if total_expected_cases.max() > 0 else None,
                fig_pinf=fig_pinf if total_pinf.max() > 0 else None,
                fig_risk=fig_risk if total_annual_risk.max() > 0 else None
            )
